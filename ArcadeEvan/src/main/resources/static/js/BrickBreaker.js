        ...
        if(state)
        {
            ...
            //*********************the next chunk of ifs causes the ball to bounce off the walls and the slider
             ...
             if (yBall <= 233) //looks for brick/ball contact, then changes brick status and redirects ball
              ...
              if (yBall >= 600)
              {
               if(nLives>0)
               {
                ...
                state = false;
                printLives = nLives + 1;
                   livesObj.change("Lives Left: " + nLives);
               }
               else
               {
               ...
               ctx.fillText("You Lose!",265,320,400);
                   printLives = 0;
                   livesObj.change("Lives Left: " + nLives);
                   var victoryObj = new MyCtor(document.getElementById("victoryBB"), "Loser...");
               }
              }
             ...
             else if(level < levels.length - 1)
             {
              level = level + 1;
                 printLevel = level + 1;
                 levelObj.change("Level: " + printLevel);
             ...
             else
             {
               ...
               ctx.fillText("You Win!",265,320,400);
                 livesObj.change("Lives Left: " + nLives);
                 var victoryObj = new MyCtor(document.getElementById("victoryBB"), "Winner!");
             }
        }
    else
     {
...
function MyCtor(element, data) { //constructor for object that holds on to data changes and subsequently changes element values 
    this.data = data;
...
printLives = nLives + 1;
var livesObj = new MyCtor(document.getElementById("livesLeft"), "Lives Left: " + nLives);
printLevel = level + 1;
var levelObj = new MyCtor(document.getElementById("level"), "Level: " + printLevel);
